...
已按 root 通知重读更新后的 `.trellis/spec/aether-gateway/backend/chat-failover.md`。HTTP 成功真实 headers 解除首次等待、900秒独立逻辑期限、旧值兼容、request/pump 所有权、usage lifecycle、取消中性归因均与批准 PRD 一致。health 公式、探针规则、其它协议及既有 incomplete 约束保留；文档没有把待执行验收宣称为通过。
