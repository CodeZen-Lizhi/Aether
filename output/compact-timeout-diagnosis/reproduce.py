"""Synthetic localhost-only reproduction; DIAG_GATEWAY_BINARY selects the tested binary."""
import base64, hashlib, http.client, json, os, pathlib, socket, sqlite3, subprocess, threading, time, uuid
...
insert('providers',id='provider-local',name='local-upstream',provider_type='custom',billing_type='free_tier',stream_first_byte_timeout=float(os.environ.get('DIAG_FIRST_TIMEOUT','0.2')),config=json.dumps({'failover_rules':{'max_attempts':1,'stream_failover_budget_ms':int(os.environ.get('DIAG_BUDGET_MS','5000'))}}))
...
try:
 ...
 usage=[dict(r) for r in check.execute('select request_type,status,status_code,error_message from usage')]; print('USAGE',json.dumps(usage),flush=True)
 report={'gateway_binary':binary,'usage':usage,'results':result,'upstream_receipts':receipts,'candidates':candidates,'health':health}
