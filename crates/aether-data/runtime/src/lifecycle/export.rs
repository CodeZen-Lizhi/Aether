...
impl ExportDomain {
    #[cfg(feature = "sqlite")]
    fn ensure_supported(self) -> Result<(), DataLayerError> {
        if matches!(
            self,
            Self::OAuthProviders | Self::UserOAuthLinks | Self::UserGroups | Self::UserGroupMembers
        ) {
            return Err(DataLayerError::InvalidInput(format!(
                "export domain '{}' has been retired and is not supported by the current SQLite schema",
                self.as_str()
            )));
        }
        Ok(())
...
fn normalize_imported_binary(
    ...
    match value {
        ...
        Value::String(value) => {
            ...
            if !encoded.is_ascii() {
                return Err(invalid("contains non-ASCII hex data"));
            }
...
pub async fn export_database_jsonl(
    ...
    let _ = (&domains, created_at_unix_secs);
    match database.driver {
        ...
        DatabaseDriver::Mysql => Err(DataLayerError::InvalidInput(
            "MySQL is not supported by aether-data; use SQLite".to_string(),
        )),
        DatabaseDriver::Postgres => Err(DataLayerError::InvalidInput(
            "PostgreSQL is not supported by aether-data; use SQLite".to_string(),
        )),
    }
}
