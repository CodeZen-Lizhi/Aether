...
fn push_selected_rows_order(builder: &mut QueryBuilder<'_, Sqlite>, order: SelectedRowsOrder) {
    ...
    builder.push("provider_id ASC, endpoint_id ASC, key_id ASC, model_id ASC");
}
...
mod tests {
    ...
    use aether_data_contracts::repository::candidate_selection::{
        MinimalCandidateSelectionReadRepository, StoredRequestedModelCandidateRowsQuery,
    };
    ...
    async fn sqlite_repository_reads_candidate_selection_rows() {
        ...
        let unscoped_requested = repository
            .list_for_exact_api_format_and_requested_model_page(
                &StoredRequestedModelCandidateRowsQuery {
                    api_format: "openai:chat".to_string(),
                    requested_model_name: "provider-model".to_string(),
                    offset: 0,
                    limit: 10,
                },
            )
            .await
            .expect("default provider model rows should load");
        assert_eq!(
            unscoped_requested
                .iter()
                .map(|row| row.key_id.as_str())
...
VALUES
  ...
  ('provider-oauth', 'OAuth Provider', 'custom', 1, 1, 1);

INSERT INTO provider_endpoints (
  id, provider_id, name, base_url, api_format, is_active, created_at, updated_at
)
VALUES
  ('endpoint-1', 'provider-1', 'Endpoint One', 'https://example.test', 'openai:chat', 1, 1, 1),
  ('endpoint-oauth', 'provider-oauth', 'OAuth Endpoint', 'https://oauth.example.test', 'openai:chat', 1, 1, 1);

INSERT INTO provider_api_keys (
  id, provider_id, name, auth_type, api_formats, is_active, created_at, updated_at
)
VALUES
  ('key-1', 'provider-1', 'Key One', 'api_key', '["openai:chat"]', 1, 1, 1),
  ('key-2', 'provider-1', 'Key Two', 'api_key', '["openai:chat"]', 1, 1, 1),
  ('key-oauth', 'provider-oauth', 'OAuth Key', 'oauth', '["openai:chat"]', 1, 1, 1);
...
VALUES
  ...
  (
    'model-oauth', 'provider-oauth', 'global-1', 'provider-model',
    NULL, 1, 1, 1, 1, 1
  );
"#,
        )
...
VALUES (
  'provider-pagination', 'Pagination Provider', 'custom', 1, 1, 1
);
...
INSERT INTO provider_api_keys (
  id, provider_id, name, auth_type, api_formats,
