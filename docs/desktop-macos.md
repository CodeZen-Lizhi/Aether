...
正常打开应用时，网关在后台准备，就绪后直接显示仪表盘，不先显示启动页。启动期间重复点击应用图标会等待同一次初始化。登录自启保持后台运行；网关故障时仅显示恢复信息，端口、自启动和目录管理统一在后台的“系统设置”中完成。

菜单栏图标和应用菜单提供打开后台、启动、停止、重启、数据目录、日志目录与退出。后台右上角的网关状态菜单提供同样的启停与重启操作。停止网关不会再弹出独立恢复页；已打开的后台保留右上角控制，端口、自启动、目录和诊断日志统一位于后台“系统设置”的“桌面应用”分区。关闭窗口会隐藏窗口并保持网关运行；菜单“退出 Aether”或 `⌘Q` 才会停止网关并退出。更换应用安装路径后，重新关闭再开启登录自启以更新路径。
...
端口被占用时，客户端会记录错误且不终止占用进程；端口修正位于后台“系统设置”的“桌面应用”分区。正常运行时可在后台修改端口；保存会由宿主安全停止网关、切换端口并重新打开后台。诊断日志也在该分区查看，完整记录可从日志目录打开。网关意外退出时会关闭失效的后台窗口，但不会弹出独立恢复页，可从菜单栏重启。正常退出先停止接收新请求，再等待在途请求与用量落库；总预算为 20 秒，超长请求可能在连接等待期限结束后被取消，日志会记录超时。宿主异常退出会通过 stdin EOF 通知受管网关退出。
...
该命令先构建 Vue 和调试版网关，再运行 Tauri。桌面宿主窗口保持隐藏，受管管理后台只允许访问当前网关端口的桌面控制命令，未向其他 HTTP 页面开放。修改前端或网关后重新运行此命令；Tauri 自身的 Rust 文件由开发进程监听。
