...
正常打开应用时，网关在后台准备，就绪后直接显示仪表盘，不先显示“正在启动网关 / 客户端设置”。启动期间重复点击应用图标会等待同一次初始化。登录自启保持后台运行；手动打开设置或遇到启动故障时才显示客户端设置页。
...
该命令先构建 Vue 和调试版网关，再运行 Tauri。客户端设置页始终使用内置来源，未向 HTTP 开发服务器开放桌面权限。修改前端或网关后重新运行此命令；Tauri 自身的 Rust 文件由开发进程监听。
