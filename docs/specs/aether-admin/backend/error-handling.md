...
通用要求见 [共享规范](../../backend/error-handling.md)；以下保留项目已确认的专门规则。

### Convention: provider-ops 动作失败时透传上游业务 message（2026-09，任务 09-02-newapi-auth-split-balance-errors）

**What**: [query_balance/mod.rs](../../../../apps/aether-gateway/src/handlers/admin/provider/ops/providers/actions/query_balance/mod.rs) 等 provider-ops 动作把上游非 2xx 响应映射为面向前端的错误时，若响应体是 JSON object 且含非空字符串 `message` 字段，必须以「固定前缀文案：上游 message」的形式拼接返回；无 `message` 或体非 JSON 时回退固定文案。
