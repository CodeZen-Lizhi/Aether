...
pub(crate) fn build_admin_update_provider_key_record_with_existing_keys(
    ...
    let managed_fixed_oauth_key = false;
    ...
    if fields.contains("internal_priority") {
        updated.internal_priority =
            normalize_internal_priority(payload.internal_priority)?.unwrap_or(50);
    }
