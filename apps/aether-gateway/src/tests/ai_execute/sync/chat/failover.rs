...
async fn gateway_retries_next_local_openai_chat_sync_candidate_after_auth_failure_impl() {
    ...
    let candidate_selection_repository =
        Arc::new(InMemoryMinimalCandidateSelectionReadRepository::seed(vec![
            ...
            sample_candidate_row(
                "provider-openai-local-secondary",
    ...
    let provider_catalog_repository = Arc::new(InMemoryProviderCatalogReadRepository::seed(
        vec![
            ...
            sample_provider_catalog_provider("provider-openai-local-secondary", "openai"),
        ],
        vec![
    ...
    .collect();
    let expected_upstream_models: std::collections::BTreeMap<&str, &str> = [
        (
            "https://api.openai.primary.example/chat/completions",
            "gpt-5-upstream-primary",
        ),
    ...
    .into_iter()
    .collect();
    let first = &seen_execution_runtime_requests[0];
    let second = &seen_execution_runtime_requests[1];
    assert_ne!(first.url, second.url);
    assert_eq!(
        first.authorization,
        expected_upstream_credentials[first.url.as_str()],
        "first attempt should use the first-ranked candidate's own credentials"
    );
    assert_eq!(
    ...
    assert_eq!(
        ...
        "retry request should carry the second candidate's own upstream model"
    );
    let stored_candidates = request_candidate_repository
