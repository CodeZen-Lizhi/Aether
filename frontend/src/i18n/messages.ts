...
const legacyExactEnglishMessages: Record<string, string> = {
  ...
  '此处仅管理默认范围映射；按端点或请求设置的映射请在单条编辑中调整。': 'Only default-scope mappings are managed here. Adjust endpoint- or request-scoped mappings in single-item editing.',
...
const legacyDynamicPatterns: Array<[RegExp, (match: RegExpMatchArray) => string]> = [
  ...
  [/^已保存 (\d+) 个客户端模型，(\d+) 个客户端模型的更改仍待保存。(.+)$/u, match => `${match[1]} client models saved; ${match[2]} client models still have pending changes. ${translateLegacyText(match[3], 'en-US')}`],
  [/^(\d+) 个客户端模型的更改保存失败，仍待保存。(.+)$/u, match => `Changes for ${match[1]} client models failed to save and remain pending. ${translateLegacyText(match[2], 'en-US')}`],
