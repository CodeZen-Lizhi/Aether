...
const legacyExactEnglishMessages: Record<string, string> = {
  ...
  '逐个选择客户端模型，并为每个模型选择多个提供商模型': 'Select each client model, then choose multiple provider models for it',
  '选择客户端': 'Select client',
  '选择提供商模型': 'Select provider models',
  '统一保存': 'Save all',
  ...
  '切换到其他客户端后，当前选择会保留为草稿。': 'Switching to another client preserves the current selection as a draft.',
  ...
  '可多选；每个客户端模型各自保留一组待保存映射。': 'Select multiple; each client model keeps its own pending mappings.',
  ...
  '映射草稿': 'Mapping drafts',
  '已暂存的映射将在保存时统一提交。': 'Staged mappings are submitted together when you save.',
  '清空草稿': 'Clear draft',
  '暂无待保存映射': 'No mappings pending save',
  '先选择客户端模型和提供商模型，草稿会显示在这里。': 'Select a client model and provider models; drafts will appear here.',
...
const legacyDynamicPatterns: Array<[RegExp, (match: RegExpMatchArray) => string]> = [
  ...
  [/^已暂存客户端 (\d+)$/u, match => `${match[1]} clients staged`],
  [/^选择 (.+)$/u, match => `Select ${match[1]}`],
  [/^待保存 (\d+)$/u, match => `${match[1]} pending`],
  [/^保存 (\d+) 条映射$/u, match => `Save ${match[1]} mappings`],
  [/^(\d+) 个客户端模型，(\d+) 条映射待保存$/u, match => `${match[1]} client models, ${match[2]} mappings pending`],
  [/^继续编辑 (.+)$/u, match => `Continue editing ${match[1]}`],
  [/^清空 (.+) 的草稿$/u, match => `Clear the draft for ${match[1]}`],
