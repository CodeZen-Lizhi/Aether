...
const legacyExactEnglishMessages: Record<string, string> = {
  ...
  '查看趋势数据': 'View trend data',
  '输入 Token 沿用上游口径，部分格式包含缓存，各项不直接相加。': 'Input tokens follow the upstream definition and include cached tokens in some formats. These categories should not be added together.',
  '使用趋势加载失败，请重试。': 'Usage trends could not be loaded. Please retry.',
  '所选时间内暂无使用数据': 'No usage data in the selected period',
  '使用趋势：费用、缓存创建、缓存命中、输入和输出。完整数值可在下方查看趋势数据。': 'Usage trends: cost, cache creation, cache hits, input and output. Exact values are available in the trend data below.',
