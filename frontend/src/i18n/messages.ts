...
const legacyExactEnglishMessages: Record<string, string> = {
  ...
  '选择客户端模型，并为它们指定一个提供商模型': 'Select client models and assign one provider model to them',
  '每次选择一个提供商模型生成配对草稿，保存前可逐项检查。': 'Select one provider model at a time to create draft pairs, then review each pair before saving.',
  '选择客户端': 'Select clients',
  '选择目标': 'Choose target',
  '保存草稿': 'Save drafts',
  '目标：': 'Target:',
  '客户端模型': 'Client model',
  '客户端模型列表': 'Client model list',
  '可多选，待保存目标会显示在每一行': 'Select one or more; each pending target appears in its row',
  '可多选，点击整行即可选择': 'Select multiple clients by clicking anywhere on a row',
  '搜索客户端模型': 'Search client models',
  '搜索客户端模型...': 'Search client models...',
  '暂无客户端模型': 'No client models',
  '无匹配客户端模型': 'No matching client models',
  '尚未配置默认范围映射': 'No default-scope mapping configured',
  '待保存': 'Pending',
  '清空已选': 'Clear selection',
  '清空已选客户端': 'Clear selected clients',
  '全选当前': 'Select all visible',
  '清除草稿': 'Clear draft',
  '提供商模型': 'Provider model',
  '提供商模型列表': 'Provider model list',
  '一次选择一个目标模型': 'Select one target model at a time',
  ...
  '点击右上角刷新按钮获取模型': 'Use the refresh button above to fetch models',
  '作用范围': 'Scope',
  '全部端点和请求': 'All endpoints and requests',
  '尚未选择目标模型': 'No target model selected',
  '默认作用于全部端点和请求': 'Applies to all endpoints and requests by default',
  '按名称自动匹配': 'Auto-match by name',
  '应用到已选客户端': 'Apply to selected clients',
  '批量添加默认作用于全部端点和请求，精细范围可在保存后通过单条编辑调整。': 'Batch additions apply to all endpoints and requests by default. Use single-item editing after saving to adjust the scope.',
  '没有待保存草稿': 'No pending drafts',
  '有未保存的映射草稿，确定要关闭吗？': 'There are unsaved mapping drafts. Close anyway?',
  ...
  '没有需要新增的映射，重复项已忽略': 'No mappings need to be added; duplicates were ignored',
  '当前列表没有可确定的同名映射': 'No deterministic name matches were found in the current list',
  '映射已存在，无需重复保存。': 'The mapping already exists; no save is needed.',
...
const legacyDynamicPatterns: Array<[RegExp, (match: RegExpMatchArray) => string]> = [
  ...
  [/^已选客户端 (\d+)$/u, match => `${match[1]} clients selected`],
  [/^已选择 (\d+) 个客户端$/u, match => `${match[1]} clients selected`],
  [/^选择 (.+)$/u, match => `Select ${match[1]}`],
  [/^待保存 (\d+)$/u, match => `${match[1]} pending`],
  [/^默认映射 (\d+)$/u, match => `${match[1]} default mappings`],
  [/^保存 (\d+) 条映射$/u, match => `Save ${match[1]} mappings`],
  [/^应用到 (\d+) 个客户端$/u, match => `Apply to ${match[1]} clients`],
  [/^(\d+) 个客户端模型待保存$/u, match => `${match[1]} client models pending`],
  [/^使用自定义提供商模型“(.+)”$/u, match => `Use custom provider model "${match[1]}"`],
  [/^清除 (.+) 的待保存映射$/u, match => `Clear the pending mapping for ${match[1]}`],
  [/^已生成 (\d+) 条映射草稿$/u, match => `${match[1]} mapping drafts created`],
  [/^已按名称生成 (\d+) 条映射草稿$/u, match => `${match[1]} mapping drafts created by name`],
  [/^已成功保存 (\d+) 条映射，可继续添加，完成后点击关闭。$/u, match => `${match[1]} mappings saved. Continue adding, then select Close when finished.`],
  [/^已保存 (\d+) 条，(\d+) 条失败并保留草稿。(.+)$/u, match => `${match[1]} saved; ${match[2]} failed and remain as drafts. ${translateLegacyText(match[3], 'en-US')}`],
  [/^(\d+) 条映射保存失败，草稿已保留。(.+)$/u, match => `${match[1]} mappings failed to save and remain as drafts. ${translateLegacyText(match[2], 'en-US')}`],
