...
const legacyExactEnglishMessages: Record<string, string> = {
  ...
  'HTTP 流式首字：从本次执行开始到网关观察到首批响应体数据或流事件，可能包含启动或心跳事件；成功响应头到达和有效正文生成是不同时间点。端到端指标包含此前调度与重试等待，缺失时回退到单轮指标。': 'HTTP streaming first-byte time runs from the start of this attempt until the gateway observes the first response-body data or stream event, which may be a startup event or heartbeat. Successful response headers and useful content arrive at separate milestones. End-to-end values include preceding scheduling and retry waits; when unavailable, the attempt value is shown.',
  ...
  '记录与存储': 'Records & storage',
  '备份与恢复': 'Backup & restore',
  '高级与诊断': 'Advanced & diagnostics',
  '默认出站代理': 'Default outbound proxy',
  '用于未单独配置代理的提供商请求，包括模型调用、余额查询和 OAuth。': 'Used for provider requests without a dedicated proxy, including model calls, balance checks, and OAuth.',
  '基本信息与请求头': 'Basic information & headers',
  '完整请求与响应': 'Full request & response',
  '脱敏请求头': 'Headers to redact',
  '请求兼容': 'Request compatibility',
  '密钥默认规则': 'Default key rules',
  '默认请求限速': 'Default request rate limit',
  '请求/分钟': 'requests/min',
  'Cyber 错误继续转移': 'Continue failover on Cyber errors',
  '关闭时直接返回 Cyber Policy 错误；开启后在响应内容开始前继续尝试其他渠道，可能增加首字等待时间': 'When disabled, Cyber Policy errors return immediately. When enabled, other channels are tried before response content begins, which may delay the first token.',
  '自动清理与保留': 'Automatic cleanup & retention',
  '自动清理': 'Automatic cleanup',
  '请求与响应内容保留': 'Request & response content retention',
  '超过期限后删除请求和响应内容。': 'Request and response content is deleted after this period.',
  '请求记录保留': 'Request record retention',
  '超过期限后删除整条请求记录。': 'The entire request record is deleted after this period.',
  '详细保留策略': 'Detailed retention policy',
  '开始压缩内容': 'Start compressing content',
  '超过此天数后压缩内容，仍可查看，不代表删除。': 'Content is compressed after this many days and remains available to view.',
  '请求头保留': 'Header retention',
  '超过期限后清空请求头。': 'Headers are cleared after this period.',
  '审计日志保留': 'Audit log retention',
  '登录和操作等安全事件的保留期限。': 'Retention for security events such as logins and user actions.',
  '候选记录保留': 'Candidate record retention',
  '请求调度候选与尝试记录的保留期限。': 'Retention for routing candidates and request attempts.',
  '代理分钟指标保留': 'Proxy minute metric retention',
  '分钟级稳定性指标的保留期限。': 'Retention for minute-level stability metrics.',
  '代理小时指标保留': 'Proxy hourly metric retention',
  '长期趋势指标，不能短于分钟指标的保留期限。': 'Long-term trend metrics; retention must be at least as long as minute metrics.',
  '开始压缩内容的天数不能超过请求与响应内容保留天数。': 'Compression must start no later than the request and response content retention period.',
  '代理小时指标的保留天数不能短于分钟指标。': 'Hourly proxy metrics must be retained at least as long as minute metrics.',
  '请填写有效的保留天数。': 'Enter a valid retention period.',
  '按策略清理': 'Clean up by policy',
  '使用已保存的保留策略清理过期数据。': 'Remove expired data using the saved retention policy.',
  '加载清理记录失败': 'Failed to load cleanup history',
  '刷新清理记录': 'Refresh cleanup history',
  ...
  '诊断': 'Diagnostics',
  '启动与端口': 'Startup & port',
  '保存失败，修改已保留，请重试。': 'Save failed. Your changes are preserved. Try again.',
  '部分配置已保存，其余修改仍待保存，请重试。': 'Some settings were saved. Other changes are still pending. Try again.',
  '删除统计聚合后仅能从剩余使用记录重建，已清理的历史统计可能丢失': 'Aggregates can only be rebuilt from remaining usage records. Statistics for deleted records may be lost.',
  '确定要后台清空全部统计聚合数据吗？原始使用记录会保留，但仅能从剩余记录重建，已清理记录对应的历史统计可能永久丢失。': 'Clear all statistics aggregates in the background? Raw usage records will remain, but only remaining records can be used to rebuild aggregates. Statistics for deleted records may be permanently lost.',
  '本机网关': 'Local gateway',
  '网关状态与操作': 'Gateway status and controls',
  '连接中': 'Connecting',
  '连接失败': 'Connection failed',
  '正在读取状态…': 'Checking status…',
  '状态读取失败': 'Status check failed',
  '重新读取状态': 'Check status again',
  ...
  '正在准备本机网关': 'Preparing your local gateway',
  '首次启动会自动准备本地数据。': 'Local data is prepared automatically on first launch.',
  '正在连接本机网关': 'Connecting to your local gateway',
  '连接本机网关': 'Connect to your local gateway',
  '正在准备管理界面，连接成功后会自动进入。': 'Preparing the dashboard. It will open when connected.',
  '重新连接': 'Reconnect',
  '可在 Aether 菜单栏中检查网关状态，启动后再次连接。': 'Check the gateway in the Aether menu bar, then reconnect after starting it.',
  '无法连接本机网关。请确认网关正在运行，然后重试。': 'Cannot connect to the local gateway. Make sure it is running, then retry.',
  '本机网关返回了无法识别的会话，请重试连接。': 'The local gateway returned an unrecognized session. Reconnect to try again.',
  '无法验证本机网关的管理权限，请重试连接。': 'Cannot verify access to the local dashboard. Reconnect to try again.',
  '请在 Aether macOS 客户端中打开管理界面。': 'Open the dashboard in the Aether macOS app.',
  '首次设置': 'First-time setup',
  '正在启动': 'Starting',
  '正在停止': 'Stopping',
  '已停止': 'Stopped',
  '运行异常': 'Gateway error',
  '正在启动网关': 'Starting your gateway',
  '网关正在运行': 'Your gateway is running',
  '正在安全停止网关': 'Stopping your gateway safely',
  '网关已停止': 'Your gateway is stopped',
  '网关需要处理': 'Your gateway needs attention',
  '正在准备本地服务，通常只需几秒。': 'Preparing the local service. This usually takes a few seconds.',
  '已准备好接收这台 Mac 上的请求。': 'Ready to receive requests from this Mac.',
  '正在等待在途请求结束并保存数据。': 'Waiting for active requests to finish and saving your data.',
  '配置与数据已保留，随时可以再次启动。': 'Your settings and data are saved. Start again whenever you need.',
  '查看下方错误信息，处理后可以重新启动。': 'Review the error below, resolve it, and start again.',
  '状态不可用': 'Status unavailable',
  '正在连接': 'Connecting',
  '无法确认网关状态': 'Cannot confirm gateway status',
  '正在读取网关状态': 'Checking gateway status',
  '请重试连接，或退出并重新打开 Aether。': 'Retry the connection, or quit and reopen Aether.',
  '正在连接 Aether 的本地服务。': 'Connecting to the Aether local service.',
  '仅本机访问': 'Local access only',
  'API 基础地址': 'API base URL',
  '地址已复制': 'Address copied',
  '复制 API 基础地址': 'Copy API base URL',
  '复制失败，请选中地址手动复制。': 'Copy failed. Select the address and copy it manually.',
  '正在打开…': 'Opening…',
  '打开管理界面': 'Open dashboard',
  '正在启动…': 'Starting…',
  '正在停止…': 'Stopping…',
  '启动网关': 'Start gateway',
  '重启': 'Restart',
  ...
  '诊断日志': 'Diagnostics',
  '最近 100 行，完整记录可在日志目录查看。': 'The latest 100 lines. Full records are available in the log folder.',
  '读取中…': 'Reading…',
  '刷新日志': 'Refresh logs',
  '最近的诊断日志': 'Recent diagnostic logs',
  '正在读取日志…': 'Reading logs…',
  '暂无诊断日志。': 'No diagnostic logs yet.',
  '关闭窗口后，网关继续在菜单栏运行。': 'Closing the window keeps the gateway running in the menu bar.',
  '退出并停止网关': 'Quit and stop gateway',
  ...
  '客户端返回了无法识别的状态，请退出并重新打开 Aether。': 'The client returned an unrecognized status. Quit and reopen Aether.',
  '请在 Aether macOS 客户端中打开此页面。浏览器无法管理本机网关。': 'Open this page in the Aether macOS app. A browser cannot manage the local gateway.',
  '无法读取诊断日志，请重试或打开日志目录。': 'Cannot read diagnostic logs. Retry or open the log folder.',
  '操作未完成，请重试或查看诊断日志。': 'The operation did not complete. Retry or check the diagnostic logs.',
  '请输入 1024–65535 之间的整数端口。': 'Enter an integer port from 1024 to 65535.',
  ...
  "导入失败：服务端返回了错误，请检查目标系统中的数据后重试": "Import failed: the server returned errors. Check the data in the target system before retrying.",
  "遇到冲突时停止导入，整次导入都会撤销": "Stop on a conflict and roll back the entire import.",
  "相同的 API Keys 按所选模式处理。任何数据导入失败时，整次导入都会撤销。": "Matching API keys follow the selected mode. If any data fails to import, the entire import is rolled back.",
  ...
  "未包含管理员资料，将保留当前管理员": "No admin profile is included. The current admin will be retained.",
  "还原备份中的配置与 API Keys，保留当前管理员资料": "Restore the backed-up settings and API keys, and retain the current admin profile.",
  "任何数据导入失败时，整次导入都会撤销。密钥会使用目标系统的加密配置重新加密；管理员密码变化后，需使用备份中的账号密码重新登录。备份包含凭证，请妥善保存。": "If any data fails to import, the entire import is rolled back. Keys are encrypted again using the target system configuration. If the admin password changes, sign in again with the backed-up credentials. Store the backup securely because it contains credentials.",
  ...
  '请求已发送，但上游服务在规定时间内没有返回首个响应。': 'The request was sent, but the upstream service did not return its first response within the configured time.',
  '这通常是临时性的服务或网络问题，建议稍后重试；如果持续发生，请检查 Endpoint、代理和上游服务状态。': 'This is usually a temporary service or network issue. Try again later; if it persists, check the endpoint, proxy, and upstream service status.',
  '请求在等待上游服务响应时超时。': 'The request timed out while waiting for the upstream service.',
  '建议稍后重试；如果持续发生，请检查 Endpoint、代理和上游服务状态。': 'Try again later; if the issue persists, check the endpoint, proxy, and upstream service status.',
  '请求格式转换失败': 'Request format conversion failed',
  '请求无法无损转换为上游服务所需的格式。': 'The request could not be converted losslessly to the format required by the upstream service.',
  '请根据技术详情中的字段路径检查格式映射。': 'Check the format mapping using the field path in the technical details.',
  '上游请求过于频繁': 'Too many upstream requests',
  '上游服务触发了频率或额度限制。': 'The upstream service applied a rate or quota limit.',
  '建议稍后重试，并检查当前 Key 的配额与限流配置。': 'Try again later and check the current key\'s quota and rate-limit settings.',
  '上游鉴权失败': 'Upstream authentication failed',
  '上游服务拒绝了当前凭据或权限。': 'The upstream service rejected the current credentials or permissions.',
  '请检查 API Key、授权范围和 Endpoint 配置。': 'Check the API key, permission scope, and endpoint configuration.',
  '上游服务暂时不可用': 'Upstream service temporarily unavailable',
  '上游服务未能正常完成请求。': 'The upstream service could not complete the request.',
  '上游请求未被接受': 'Upstream request was not accepted',
  '上游服务未能处理当前请求。': 'The upstream service could not process the current request.',
  '请检查请求参数、模型名称和 Endpoint 配置。': 'Check the request parameters, model name, and endpoint configuration.',
  '请求处理失败': 'Request processing failed',
  '当前请求未能完成。': 'The current request could not be completed.',
  '请查看技术详情定位原因，并根据原始错误调整请求或服务配置。': 'Review the technical details, then adjust the request or service configuration based on the original error.',
  '技术详情': 'Technical details',
  '原始错误与响应数据': 'Original error and response data',
  '原始错误': 'Original error',
  ...
  '添加映射': 'Add mapping',
  '批量映射': 'Batch mapping',
  '批量添加模型映射': 'Add model mappings in bulk',
  '选择客户端模型，查看并修改默认范围映射': 'Select a client model to view and edit default-scope mappings',
  '配置映射': 'Configure mappings',
  '先选择一个客户端模型，再从下拉列表中多选提供商模型。': 'Select one client model, then choose multiple provider models from the dropdown.',
  '客户端模型': 'Client model',
  '请选择客户端模型': 'Select a client model',
  '切换到其他客户端后，当前选择会保留在映射列表中。': 'Switching to another client preserves the current selection in the mapping list.',
  '请先选择客户端模型': 'Select a client model first',
  '提供商模型': 'Provider model',
  '未选择': 'Not selected',
  '选择一个或多个提供商模型': 'Select one or more provider models',
  '刷新提供商模型': 'Refresh provider models',
  '搜索提供商模型...': 'Search provider models...',
  '正在加载提供商模型': 'Loading provider models',
  '暂无提供商模型': 'No provider models',
  '无匹配提供商模型': 'No matching provider models',
  '可多选；已配置的默认范围映射会在这里回显，可直接修改。': 'Select multiple; existing default-scope mappings are shown here and can be edited directly.',
  '选择客户端模型后即可多选。': 'Select a client model to enable multi-select.',
  '添加自定义提供商模型': 'Add custom provider model',
  '输入自定义提供商模型名称...': 'Enter a custom provider model name...',
  '添加并选中': 'Add and select',
  '默认作用于全部端点和请求': 'Applies to all endpoints and requests by default',
  '映射列表': 'Mapping list',
  '默认范围映射会在保存时更新。': 'Default-scope mappings update when you save.',
  '清空映射': 'Clear mappings',
  '默认范围映射已清空': 'Default-scope mappings cleared',
  '暂无默认范围映射': 'No default-scope mappings',
  '选择客户端模型并添加提供商模型，映射会显示在这里。': 'Select a client model and add provider models; mappings appear here.',
  '没有待保存更改': 'No pending changes',
  '有未保存的映射更改，确定要关闭吗？': 'There are unsaved mapping changes. Close anyway?',
  '放弃更改': 'Discard changes',
  '放弃并关闭': 'Discard and close',
  '继续编辑': 'Keep editing',
  '客户端模型不存在': 'Client model not found',
  '批量保存失败': 'Batch save failed',
  '获取提供商模型提示': 'Provider model notice',
  '获取提供商模型失败': 'Failed to load provider models',
  '获取上游模型失败': 'Failed to load upstream models',
  '点击上方“批量映射”为多个模型统一配置映射': 'Use Batch mapping above to configure several models together',
  '未设置': 'Not set',
  '已设置': 'Configured',
  '默认': 'Default',
  '系统默认': 'System default',
  ...
  '刷新节点': 'Refresh nodes',
  '正在加载代理节点...': 'Loading proxy nodes...',
  '获取代理节点列表失败': 'Failed to load proxy nodes',
  '代理节点列表不完整，请重试': 'The proxy node list is incomplete. Please retry.',
  '数据已导入，但页面刷新失败，请刷新页面查看最新数据': 'Data was imported, but the page could not refresh. Reload the page to view the latest data.',
  ...
  '计费类型': 'Billing type',
  '最大重试次数': 'Max retries',
  '总尝试次数（含首次）': 'Total attempts (including first)',
  '当前生效次数': 'Effective attempts',
  '流式请求总超时': 'Streaming request total timeout',
  '当前生效总超时': 'Effective total timeout',
  '(毫秒)': '(ms)',
  '故障转移规则': 'Failover rules',
  '端点配置': 'Endpoint settings',
  '提供商配置': 'Provider settings',
  '默认配置': 'Defaults',
  '总尝试次数必须是 1 到 99 之间的整数': 'Total attempts must be an integer from 1 to 99',
  '流式请求总超时必须在 1 到 1200 秒之间，最多保留三位小数': 'Streaming request total timeout must be between 1 and 1200 seconds, with up to three decimal places',
  '从请求开始到完整回答结束，包含重试、切换和输出；普通流式与压缩共用。清空恢复默认 900 秒。': 'From request start to the complete answer, including retries, failover and output; shared by chat streams and compaction. Clear to restore the 900-second default.',
  '每次尝试等待上游成功响应头的上限；收到响应头后，继续等待结果由流式请求总超时限制。': 'Limits the wait for successful upstream response headers on each attempt. After headers arrive, the streaming request total timeout limits the remaining wait.',
  '默认 2': 'Default 2',
  '最大转移次数': 'Max transfers',
  '最大转移超时': 'Max transfer timeout',
  '0 (不限制)': '0 (unlimited)',
  '首次响应超时': 'First response timeout',
  ...
  '1 × 表示不调整': '1 × means no adjustment',
  '并发不限': 'No concurrency limit',
  ...
  '已启用请求 gzip（点击关闭）': 'Request gzip enabled (click to disable)',
  '启用请求 gzip': 'Enable request gzip',
  '已启用请求 gzip': 'Request gzip enabled',
  '已关闭请求 gzip': 'Request gzip disabled',
  '更新请求 gzip 失败': 'Failed to update request gzip',
  ...
  '同步失败': 'Sync failed',
  '自动同步': 'Auto sync',
  '同步在线价格': 'Sync online prices',
  '同步中...': 'Syncing...',
  '批量同步模型价格': 'Batch sync model prices',
  '同步价格': 'Sync prices',
  '同步全部模型价格': 'Sync prices for all models',
  '确认清除': 'Confirm clear',
  ...
  'Key 成本倍率：该密钥所有请求按此计费': 'Key cost multiplier: all requests for this key are billed at this rate',
  ...
  '今日平均响应': 'Average response today',
  '今日请求的平均耗时': 'Average duration of today’s requests',
  '今日暂无请求': 'No requests today',
  '暂无响应时间数据': 'No response time data',
  '累计 Token': 'Lifetime tokens',
  '累计请求': 'Lifetime requests',
  '累计费用': 'Lifetime cost',
  '全部历史': 'All time',
  '统计起始': 'Since',
  '暂无使用数据': 'No usage yet',
  ...
  '今日费用': 'Cost today',
  '全站 RPM / TPM': 'Site RPM / TPM',
  '最近 60 秒': 'Last 60 seconds',
  '累计统计加载失败': 'Lifetime statistics could not be loaded',
  '今日统计加载失败': 'Today’s statistics could not be loaded',
  '重试累计统计': 'Retry lifetime statistics',
  '重试今日统计': 'Retry today’s statistics',
  '用量分布': 'Usage breakdown',
  '按所选统计周期汇总': 'Totals for the selected period',
  '排行排序': 'Rank by metric',
  '用量分布加载失败': 'Usage breakdown could not be loaded',
  '模型用量': 'Usage by model',
  '提供商用量': 'Usage by provider',
  '该周期未保留分组明细': 'No group details were retained for this period',
  '收起列表': 'Collapse list',
  '失败率': 'Failure rate',
  ...
  '时段': 'Period',
  '明细缺失': 'Details unavailable',
  '使用趋势加载失败，请重试。': 'Usage trends could not be loaded. Please retry.',
  '所选时间内暂无使用数据': 'No usage data in the selected period',
  '使用趋势：费用、缓存创建、缓存命中、输入和输出。悬停可查看缓存命中率。': 'Usage trends: cost, cache creation, cache hits, input and output. Hover to see the cache hit rate.',
  '部分历史记录未保留请求明细，无法还原小时趋势；切换为按天可查看保留的费用。': 'Some historical request details are unavailable, so hourly trends cannot be reconstructed. Switch to daily view to see retained costs.',
  '部分历史记录未保留 Token 明细，对应曲线留空，已保留的费用仍完整展示。': 'Some historical token details are unavailable. Those curves contain gaps, while retained costs are still shown in full.',
  '1h 缓存': '1h cache',
  '上限像素': 'Max pixels',
  ...
  '未保留模型明细': 'Model details not retained',
  '未保留提供商明细': 'Provider details not retained',
  '部分历史记录未保留模型或提供商明细，总请求和总费用已保留，缺失部分单独标注。': 'Some historical records lack model or provider details. Request and cost totals are retained, with missing details shown separately.',
...
const legacyPhraseEnglishMessages: Array<[string, string]> = [
  ...
  ['在线目录暂时无法更新，当前展示上次成功加载的缓存。', 'The online catalog cannot be updated right now. Showing the last successfully loaded cache.'],
  ['在线模型目录加载失败', 'Failed to load the online model catalog'],
  ['请检查外部模型目录代理，或稍后重试。', 'Check the external model catalog proxy or try again later.'],
  ['Key优先', 'Key first'],
  ...
  ['勾选Key可以使用的API格式，同时可以设置key访问该端点时的倍率。', 'Select the API formats this key can use; the cost multiplier is configured per key.'],
  ['RPM限制：', 'RPM limit:'],
  ...
  ['支持的API格式：勾选Key可以使用的API格式，同时可以设置key访问该端点时的倍率。', 'Supported API formats: select the API formats this key can use; the cost multiplier is configured per key.'],
...
const legacyDynamicPatterns: Array<[RegExp, (match: RegExpMatchArray) => string]> = [
  ...
  [/^选择 (.+)$/u, match => `Select ${match[1]}`],
  [/^(\d+) 个客户端模型有待保存更改$/u, match => `${match[1]} client models have pending changes`],
  [/^保存 (\d+) 项更改$/u, match => `Save ${match[1]} changes`],
  [/^继续编辑 (.+)$/u, match => `Continue editing ${match[1]}`],
  [/^清空 (.+) 的映射$/u, match => `Clear mappings for ${match[1]}`],
  [/^移除 (.+)$/u, match => `Remove ${match[1]}`],
  [/^已成功保存 (\d+) 个客户端模型的映射。$/u, match => `Saved mappings for ${match[1]} client models.`],
  [/^已保存 (\d+) 个客户端模型，(\d+) 个客户端模型的更改仍待保存。$/u, match => `${match[1]} client models saved; ${match[2]} client models still have pending changes.`],
  [/^(\d+) 个客户端模型的更改保存失败，仍待保存。$/u, match => `Changes for ${match[1]} client models failed to save and remain pending.`],
  [/^同步在线价格 \((\d+)\)$/u, match => `Sync online prices (${match[1]})`],
  [/^同步选中的 (\d+) 个模型价格$/u, match => `Sync prices for ${match[1]} selected models`],
  [/^将根据各模型上次选择的提供商的在线定价更新(选中的|全部) (\d+) 个模型。(?:另有 (\d+) 个模型因价格一致、计价不兼容或无在线价格而跳过。)?\n\n仅更新模型价格，不修改名称、能力或其他配置。$/u, match => `Will update ${match[2]} ${match[1] === '选中的' ? 'selected' : 'all'} models based on each model's last-selected provider's online pricing.${match[3] ? ` Another ${match[3]} models are skipped (identical pricing, incompatible pricing model, or no online price).` : ''}\n\nOnly model prices are updated; names, capabilities, and other settings are untouched.`],
